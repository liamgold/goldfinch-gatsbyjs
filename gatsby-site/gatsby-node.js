...
exports.onCreateNode = ({ node, actions }) => {
  ...
  if (node.internal.type.match(/KenticoCloudItem/)) {
    ...
    if (
      node.internal.type ===
      `${kcItemTypeIdentifier}${projectReferenceTypeIdentifier}`
    ) {
      templateName = `project-reference`;
      withDetailView = true;
    } else if (
      node.internal.type ===
      `${kcItemTypeIdentifier}${speakingEngagementTypeIdentifier}`
    ) {
  ...
  return new Promise((resolve) => {
    ...
    `).then(result => {
      const union = new Array(
        ...result.data.allKenticoCloudItemProjectReference.edges,
        ...result.data.allKenticoCloudItemSpeakingEngagement.edges
      );
